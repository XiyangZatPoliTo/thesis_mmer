...
def plot_predicted_example(batch, model, n_rows=8, n_cols=4, figsize=(20, 20), title_font_size=20):
    ...
    for img_ind in range(n_rows * n_cols):
        ax[img_ind // n_cols][img_ind % n_cols].imshow(
            torch.flip((x[img_ind, :, :, :L[img_ind]].squeeze().transpose(0, 1)), dims=[0]))
        ax[img_ind // n_cols][img_ind % n_cols].set_title(
            emotion_dict[f"0{y[img_ind].item() + 1}"] + "/" + emotion_dict[
                f"0{y_pred[img_ind] + 1}"] + "({p:.2f} %)".format(p=y_prob[img_ind].numpy().max() * 100),
...
def plot_cm(cm, normalize=False, ax=None):
    ...
    cm_df = pd.DataFrame(cm, columns=[emotion_dict[f"0{i}"] for i in range(1, 9)],
                         index=[emotion_dict[f"0{i}"] for i in range(1, 9)])
